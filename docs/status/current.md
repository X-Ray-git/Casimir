...
- 根据用户提供的首页与带 PDF 附件的两份 MHTML，适配新表单、输入框、通用文件选择器、
  附件卡片和侧边栏按钮；保留旧布局，后台 debugger 聚焦表达式同步更新。
- `npm run check` 通过，共 76 项测试；两份原始快照离线选择器核对通过。
- 用户实测发现重复 PDF 和确认超时。已取消超时自动重传并改为未确认提示。
  故障快照确认卡片名称增加 `(5)` 后缀；已兼容数字后缀并排除既有卡片，用户最终确认出现“PDF 已添加”提示，附件流程验收通过。
  新版快捷键与原生键盘焦点尚未逐项重新实测。
- 结构证据、测试与限制见 [布局适配记录](../operations/2026-09-25-chatgpt-layout.md)。
